...
impl GicV3 {
    ...
    pub fn setup(&mut self) {
        unsafe {
            // init gicd
            addr_of_mut!((*self.gicd).ctlr).write_volatile(GicdCtlr::empty());
            loop {
                let rwp = addr_of_mut!((*self.gicd).ctlr).read_volatile().bits() & 1 << 31;
                if rwp == 0 {
                    break;
                }
            }
            for i in 0..32 {
                ...
                addr_of_mut!((*self.gicd).icpendr[i]).write_volatile(0xffffffff);
                addr_of_mut!((*self.gicd).igroupr[i]).write_volatile(0xffffffff);
                addr_of_mut!((*self.gicd).igrpmodr[i]).write_volatile(0xffffffff);
            }
            for i in 0..1024 {
                addr_of_mut!((*self.gicd).ipriorityr[i]).write_volatile(0xa0);
            }
            for i in 0..64 {
                addr_of_mut!((*self.gicd).icfgr[i]).write_volatile(0x0);
            }
            addr_of_mut!((*self.gicd).ctlr)
                .write_volatile(GicdCtlr::ARE_S | GicdCtlr::EnableGrp1NS);
            let mpidr = read_sysreg!(mpidr_el1) & 0x000000ff00ffffff;
            for i in 0..987 {
                addr_of_mut!((*self.gicd).irouter[i]).write_volatile(mpidr);
            }
        }
    }

    /// Initialises the CPU interface of the GIC for the current core.
    pub fn init_cpu(&mut self) {
        unsafe {
            ...
            addr_of_mut!((*self.sgi).icpendr0).write_volatile(0xffffffff);
            addr_of_mut!((*self.sgi).igroupr0).write_volatile(0xffffffff);
            addr_of_mut!((*self.sgi).igrpmodr0).write_volatile(0xffffffff);
            for i in 0..32 {
                addr_of_mut!((*self.sgi).ipriorityr[i]).write_volatile(0xa0);
            }
            for i in 0..2 {
                addr_of_mut!((*self.sgi).icfgr[i]).write_volatile(0x0);
            }

            // init gicc
            let reg = read_sysreg!(icc_sre_el1);
            if (reg & 1 << 0) != 0 {
                write_sysreg!(icc_sre_el1, reg | 0b111);
            }
