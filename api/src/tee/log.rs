...
use starry_core::task::TeeSessionCtxTrait;

#[cfg(feature = "tee_test")]
use super::{tee_unit_test::tee_test_unit, test::test_examples::tee_test_example};
...
pub(crate) fn sys_tee_scn_log(buf: *const c_char, len: usize) -> AxResult<isize> {
    ...
    let concrete = lock.as_ref().unwrap().as_any().downcast_ref::<tee_session_ctx>().unwrap();
    info!(
