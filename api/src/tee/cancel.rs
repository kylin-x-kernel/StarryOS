...
pub(crate) fn sys_tee_scn_get_cancellation_flag(cancel: *mut c_uint) -> TeeResult {
...
pub(crate) fn sys_tee_scn_unmask_cancellation(old_mask: *mut c_uint) -> TeeResult {
...
pub(crate) fn sys_tee_scn_mask_cancellation(old_mask: *mut c_uint) -> TeeResult {
    let prev = with_tee_session_ctx_mut(|ctx| {
        let prev = ctx.cancel_mask;
